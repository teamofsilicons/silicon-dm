...
const origin='https://docs.dm.teamofsilicons.com';
const revision='codex/dm-understanding-0.5';
...
const groups=[['Start here',['README.md','getting-started.md','testing-environments.md']],['Use DM',['cli/README.md','cli/relay.md','configuration.md','telemetry.md']],['Build with DM',['building.md','client/README.md','client/runtime.md','client/realtime.md','api/README.md','wire-format.md','contracts.md','iam.md']],['Operate',['deployment.md','release-0.5.0.md']]];
...
for(const page of pages){
 ...
 for(let i=0;i<tokens.length;i++){
  ...
  for(const child of t.children||[])if(child.type==='link_open'){
   ...
   if(!target)target=`https://github.com/teamofsilicons/silicon-dm/blob/${revision}/${source.startsWith('../')?source.slice(3):'docs/'+source}`;
   child.attrSet('href',target+(hash?'#'+hash:''));
  }
 }
 const nav=groups.map(([title,sources])=>`<div class="nav-group"><span>${title}</span>${sources.map(source=>{const p=pages.find(p=>p.source===source);if(!p)throw Error(source);return `<a href="${p.route}"${p.route===page.route?' aria-current="page"':''}>${esc(p.title.replace(/^Silicon DM[: —-]?\s*/,''))}</a>`}).join('')}</div>`).join('');
 const body=md.renderer.render(tokens,md.options,{});
 const html=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(page.title)} · DM Docs</title><meta name="description" content="Install Silicon DM, send reliable messages, and build integrations with the CLI, Rust client, and API."><link rel="canonical" href="${origin}${page.route}"><link rel="stylesheet" href="/style.css"></head><body><a class="skip" href="#content">Skip to content</a><header><a class="brand" href="/">◈ <b>Silicon DM</b> <span>Docs</span></a><nav><a href="/getting-started/">Quickstart</a><a href="/openapi.yaml">OpenAPI ↗</a><a href="https://github.com/teamofsilicons/silicon-dm">GitHub ↗</a></nav></header><div class="layout"><aside class="sidebar"><p class="version">DM 0.5 · DOCUMENTATION</p>${nav}</aside><main id="content"><div class="crumb">DM / ${esc(page.source==='README.md'?'Overview':page.title)}</div><div class="release-note">0.5 implementation preview. Backend rollout and package publication are pending. <a href="/release-0.5.0/">Release status</a></div><article>${body}</article><footer>Team of Silicons · <a href="https://github.com/teamofsilicons/silicon-dm/blob/${revision}/docs/${page.source}">Edit this page</a><p>Usage guides first. Protocol details when you need them.</p></footer></main><aside class="toc"><span>ON THIS PAGE</span>${toc.map(([id,title])=>`<a href="#${esc(id)}">${esc(title)}</a>`).join('')}</aside></div></body></html>`;
...
await writeFile(join(out,'404.html'),'<!doctype html><html lang="en"><meta charset="utf-8"><title>Page not found · DM Docs</title><link rel="stylesheet" href="/style.css"><main><h1>Page not found</h1><p><a href="/">Return to Silicon DM documentation.</a></p></main></html>');
