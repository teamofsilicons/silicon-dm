...
impl TestingRegistry {
    ...
    async fn execute_operation(&self, op: &Operation, hash: &[u8]) -> AppResult<Value> {
        ...
        if replay.is_none() {
            ...
            sqlx::query("INSERT INTO dm.testing_environments(environment_id,organization_id,creator_actor_id,creator_actor_kind,name,description,iam_environment_id,iam_app_id,iam_environment_key_ciphertext,iam_app_secret_ciphertext,root_key_digest,root_key_ciphertext,status,iam_sync_pending,version) VALUES($1,$2,'honeycomb','silicon',$3,$4,$1,$5,'','',$6,$7,'active',true,$8) ON CONFLICT(environment_id) DO UPDATE SET name=$3,description=$4,status='active',deleted_at=NULL,purge_after=NULL,root_key_digest=$6,root_key_ciphertext=$7,iam_environment_key_ciphertext='',iam_sync_pending=true,version=$8,runtime_revision=dm.testing_environments.runtime_revision+1")
                .bind(id).bind(&op.org_id).bind(op.name.as_deref().unwrap_or("Honeycomb sandbox")).bind(&op.description).bind(&op.app_id).bind(blake3::hash(op.testing_key.as_bytes()).as_bytes().as_slice()).bind(self.encrypt(id,"root",&root)?).bind(op.generation).execute(&mut *tx).await?;
