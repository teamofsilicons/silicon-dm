...
--binaries-dir containing <honeycomb-target>/dm[.exe] instead.
...
def main() -> None:
    ...
    archive = output / f"dm-{version}.tar.gz"
    checksums = []
    expected_files = {"honeycomb.yaml"}
    with tempfile.TemporaryDirectory(prefix="dm-release-") as temporary:
    ...
    (output / f"dm-{version}-binaries.sha256").write_text("\n".join(checksums) + "\n")
